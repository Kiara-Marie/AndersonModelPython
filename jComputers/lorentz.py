...
class Constant(JComputer) :
  def __init__(self, nnOnly, t, rdep, gamma, energyComputer):
  ...
  def jFinder(self, xi, xj):
    # should only be called after this instance's energyComputer has had calculateEnergies called
    if (energyComputer.energies is None):
      raise Exception("Attempted to use jComputer which needs energy, but energy not yet computed!")
    if (self.nnOnly and (xi - xj) > 1):
      return 0
    r = 5*(xi - xj)
    denominator = 1 + ((self.energyComputer.energies[i] - self.energyComputer.energies[j])/(self.gamma))**2
